...
def targetscan_predict(seq_file, result_file):
    ...
    command = "/home/fux/tools/TargetScan/targetscan_70.pl {t_querymir_file} {t_utr_file} {result_file}".format(
        t_querymir_file=seq_file,
        t_utr_file=t_utr_file,
...
def mirmap_predict(mirseq, result_file):
    ...
    conmand = "python2 /home/fux/web/miRNASNP3/miRNASNP3/miRNASNP3/online_predict/scr/run_mirmap.py {mirseq} {result_file}".format(
        mirseq=mirseq, result_file=result_file)
    #print(conmand)
    ...
    with open(wm_bed_file, "a") as wm_bed:
        with open(wm_result) as infile:
            ...
            #    out.write(line)
                    ...
                    item['site_start'] = str(
                        int(item['site_end']) - int(item['seed_length']))
                    ...
                    item['align7'] = ' ' * 9 + ' ' * (len(line01) -
                                                      len(line03)) + line03
                    item['align8'] = "3' " + ' ' * (len(line01) -
                                                    len(line04)) + line04
                    item['site_chr'] = nline[0]
                    item['strand'] = re.split(r'\(|\)', nline[1])[1]
                    position = re.split(r'\(|\)', nline[1])[0].split('-')
                    if item['strand'] == '-':
                        item['site_genome_start'] = str(
                            int(position[1]) - int(item['site_end']))
                        item['site_genome_end'] = str(
                            int(item['site_genome_start']) +
                            int(item['seed_length']))
                    else:
                        item['site_genome_end'] = str(
                            int(position[0]) + int(item['site_end']))
                        item['site_genome_start'] = str(
                            int(item['site_genome_end']) -
                            int(item['seed_length']))
                    ...
                    bed_line = utr_info + '\t' + item[
                        'site_start'] + '\t' + item['site_end']
                    lkey = utr_info + '#' + item['site_start'] + '#' + item[
                        'site_end']
                    wm_bed_json[
                        lkey] = item  #when item change,all values of wm_bed_json changed
                    item = {}
                    #print("wm_utr_list:")
                    #print(wm_utr_list)
                    #print(bed_line)
    ...
    with open(w_insect_file) as infile:
        for line in infile:
            ...
            if nline[0] not in s_unin:
                loss.append(wm_bed_json[nline[0] + '#' + nline[1] + '#' +
                                        nline[2]])
            wild_result.append(wm_bed_json[nline[0] + '#' + nline[1] + '#' +
                                           nline[2]])
    ...
    with open(s_insect_file) as infile:
        for line in infile:
            ...
            if nline[0] not in w_unin:
                gain.append(sm_bed_json[nline[0] + '#' + nline[1] + '#' +
                                        nline[2]])
            alt_result.append(sm_bed_json[nline[0] + '#' + nline[1] + '#' +
                                          nline[2]])
    #print(gain)
    #print(loss)
    result = {
...
def predict_start(wild_seq, wild_prefix, alt_seq, alt_prefix):
    ...
    pool.apply_async(targetscan_predict, (
        wild_tgs_file,
        wt_result,
    ))
    pool.apply_async(mirmap_predict, (
        wild_seq,
        wm_result,
    ))
    pool.apply_async(targetscan_predict, (
