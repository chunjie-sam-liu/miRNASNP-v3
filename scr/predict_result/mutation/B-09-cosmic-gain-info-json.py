...
def getMutationinfo():
    with open("/home/fux/fux/miRNASNP3/mutation/cosmic_in_utr3.ty.snp.pos_correct.json") as infile:
    ...
    with open(sys.argv[1]) as infile:
        for line in infile:
            ...
            if strand =='-':
                ...
                distance=int(nutr[10])-int(mutinfo[1])
                if distance==int(nutr[10])-int(nutr[9]):
                    side_count+=1
                    continue
            else:
                curseq = list(''.join(map(lambda x:RULE2[x],nalign[2])))
                distance=int(mutinfo[1])-int(nutr[9])-1
            ...
            if mutinfo[1]>=min(nline[1],nline[5]) and mutinfo[1]<=max(nline[2],nline[6]):
                ...
                if mutinfo[1]>=nline[1] and mutinfo[1]<nline[2]:
                    if curseq[distance-int(nalign[0].split()[0])+1]==mutinfo[4]:
                        item+=1
            ...
            site_info['truncate_start']=nutr[9]
            site_info['truncate_end']=nutr[10]
